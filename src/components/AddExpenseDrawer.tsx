...
export const AddExpenseDrawer: React.FC = () => {
  ...
  const convertedBase = convertCurrency(numAmount, currency, activeTrip.baseCurrency, customRates);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex justify-center items-end animate-in fade-in duration-200">
      <div className="bg-[#161c28] dark:bg-[#161c28] light:bg-white border-t border-x border-[#28354d] rounded-t-3xl max-w-lg w-full max-h-[92vh] overflow-y-auto p-5 shadow-2xl animate-in slide-in-from-bottom duration-300">
        {/* Header Bar */}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-4">
          ...
          <div className="p-4 rounded-2xl bg-[#0e121b] border border-[#2a3750]">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-400">消费金额</span>
              <select
                value={currency}
                onChange={(e) => setCurrency(e.target.value as CurrencyCode)}
                className="bg-[#1f293d] text-white text-xs font-bold px-2.5 py-1 rounded-lg border border-[#344463] focus:outline-none focus:border-[#ff6b6b]"
              >
            ...
            <div className="flex items-baseline space-x-2 mt-2">
              ...
              <input
                ...
                className="w-full bg-transparent text-3xl font-black text-white focus:outline-none placeholder:text-slate-600"
              />
            </div>

            {/* Realtime Conversion Preview */}
            {currency !== activeTrip.baseCurrency && numAmount > 0 && (
              <div className="flex items-center space-x-1.5 text-xs text-[#06d6a0] font-bold mt-2 pt-2 border-t border-[#1f293d]">
          ...
          {/* Expense Title */}
          <div>
            <label className="text-xs font-bold text-slate-400 block mb-1">费用名称 / 备注</label>
            <input
              type="text"
              placeholder="例如：机场免税店、居酒屋晚餐、包车接送"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full px-3.5 py-2.5 rounded-xl bg-[#0e121b] text-white text-xs border border-[#2a3750] focus:outline-none focus:border-[#ff6b6b]"
            />
          </div>

          {/* Category Selector Grid */}
          <div>
            <label className="text-xs font-bold text-slate-400 block mb-1.5">支出类别</label>
                ...
                return (
                  <button
                    ...
                    className={`p-2 rounded-xl border flex flex-col items-center justify-center transition-all ${
                      isSelected
                        ? 'bg-[#ff6b6b]/20 border-[#ff6b6b] text-[#ff6b6b] font-bold shadow-xs'
                        : 'bg-[#0e121b] border-[#2a3750] text-slate-400 hover:text-white'
                    }`}
                  >
          ...
          <div>
            <label className="text-xs font-bold text-slate-400 block mb-1.5">谁付的钱？</label>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl border text-xs shrink-0 transition-all ${
                      isSelected
                        ? 'bg-white text-[#121620] font-bold border-white shadow-md'
                        : 'bg-[#0e121b] text-slate-300 border-[#2a3750] hover:border-slate-500'
                    }`}
                  >
                    <span
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-bold text-slate-400">谁参与平分？</label>
              <button
                type="button"
                ...
                return (
                  <button
                    ...
                    className={`p-2 rounded-xl border flex items-center justify-between text-xs transition-all ${
                      isChecked
                        ? 'bg-[#ff6b6b]/15 border-[#ff6b6b]/60 text-white font-bold'
                        : 'bg-[#0e121b] border-[#2a3750] text-slate-500'
                    }`}
                  >
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div>
              <label className="text-xs font-bold text-slate-400 block mb-1">消费日期</label>
              <input
                type="date"
                value={date}
            ...
            <div>
              <label className="text-xs font-bold text-slate-400 block mb-1">发票 / 小票照片</label>
              <label className="w-full py-2 px-3 rounded-xl bg-[#0e121b] text-slate-300 text-xs border border-[#2a3750] hover:border-[#ff6b6b] flex items-center justify-center space-x-1 cursor-pointer transition-colors">
